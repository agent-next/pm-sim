...
- E2E tests against live Polymarket API
 `implement` only on `bot:implement` or `/oc implement` (`deepseek-v4-flash` + `OC_PUSH_TOKEN`); `review` stays comment-only; `merge-ready` labels when Python 3.10–3.13 are green.
